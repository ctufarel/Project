...
hg19='./Annotation_hg19/hg19.bed' # change with bed annotation file of interest (here used RefSeq and UCSC)
...
bedtools intersect -wb -wa -a $@/sorted_only_tum_LCTs.bed -b $@/sorted_only_inflamed_LCTs.bed -s -f 0.5 > $@/shared_inflVStum_LCTs.bed	
# sorts shared-LCT bed file		
sort -k1,1 -k2,2n $@/shared_inflVStum_LCTs.bed > $@/final_sorted_inflVStum_LCTs.bed
# merges shared-LCT file		
bedtools merge -i $@/final_sorted_inflVStum_LCTs.bed -s > $@/final_file.bed	# it can be tracked using UCSC (Custom Track option)

## Downstream analysis: Now, one can annotate the shared-LCTs using bedtools intersect and the hg19.bed file (strand and gene name) generated by UCSC Table (NCBI-RefSeq+ALLRefSeq+Select Fields from hg19.ncbiRefSeqCurated: strand and name2 were selected):
input_file=$@/final_sorted_inflVStum_LCTs.bed
# sorts annotation file	
sort -k1,1 -k2,2n $hg19 > $@/sorted_hg19.bed	
# intersection of shared-LCT bed file and hg19 annotation file and generates annotation bed file						
bedtools intersect -wa -wb -a $input_file -b $@/sorted_hg19.bed -s -sorted -f 0.8 > $@/annot.bed
